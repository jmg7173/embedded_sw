...
void iom_fpga_fnd_write(unsigned short int num){
    unsigned int iom_fpga_fnd_addr = ioremap(FPGA_FND_ADDRESS, 0x4);
    outw(num, iom_fpga_fnd_addr);
    iounmap(iom_fpga_fnd_addr);
}

void iom_fpga_led_write(unsigned short int num){
    unsigned int iom_fpga_led_addr = ioremap(FPGA_LED_ADDRESS, 0x1);
    outw(num, iom_fpga_led_addr);
    iounmap(iom_fpga_led_addr);
}

void iom_fpga_text_lcd_write(const char* gdata){
    unsigned int iom_fpga_text_lcd_addr = ioremap(FPGA_TEXT_LCD_ADDRESS, 0x32);
    int i, length = 32;

    for(i = 0; i<length; i++){
        _s_value = (gdata[i] & 0xFF) << 8 | gdata[i+1] & 0xFF;
        outw(_s_value, iom_fpga_text_lcd_addr+i);
        i++;    
    }
